...
def get_auth_headers():
    """Get authentication headers, with fallback for local development."""
    try:
        token = get_id_token()
        return {"Authorization": f"Bearer {token}"}
    except Exception as e:
        logging.warning(f"Could not get ID token: {e}. Using no authentication for local development.")
        return {}
